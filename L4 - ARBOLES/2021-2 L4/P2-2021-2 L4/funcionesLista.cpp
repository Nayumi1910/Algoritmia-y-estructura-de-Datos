...
//void insertarAlInicio(struct Lista & tad, struct NodoArbol* elemento) {
//
...
void insertarencola(struct Lista & tad, struct NodoArbol* elemento) {
...
void eliminaNodo(struct Lista & tad, int elemento) {
    ...
    while ((recorrido != nullptr) and (recorrido->elemento->elemento != elemento)) {
        ultimo = recorrido;
        recorrido = recorrido->siguiente;
    }
...
void imprime(const struct Lista & tad) {
    ...
    if (esListaVacia(tad)) {
        ...
        while (recorrido != nullptr) {
            ...
            cout << recorrido->elemento->elemento<<"-"<<recorrido->elemento->letra<<" ";
            recorrido = recorrido->siguiente;
        }
        cout << "]" << endl;
    }
