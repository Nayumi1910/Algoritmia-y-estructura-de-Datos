...
/*Añade un elemento a la cola*/
void encolarNew(struct Cola & cola, struct NodoArbol *nodo){
    insertarencolaNew(cola.lista, nodo);
}

/*Elimina un elemento de la cola*/
NodoArbol *desencolarNew(struct Cola & cola){
    if(esColaVacia(cola)){
        //cout<<"La cola está vacía no se puede desencolar"<<endl;
        return nullptr; //Cero o algún elemento que no esté en la cola
    }
    else{
        NodoArbol *nodoArbol = retornaCabezaNew(cola.lista);
        eliminaCabeza(cola.lista);
        if(esColaVacia(cola))
            cola.lista.cola = nullptr;
        return nodoArbol;
