...
struct NodoArbol * crearNuevoNodo(struct NodoArbol * izquierda, int elemento,
        char letra,struct NodoArbol * derecha) {
...
void plantarArbolBinarioB(struct NodoArbol *& raiz, 
                    struct NodoArbol * izquierda, int elemento,
                    char letra,
                    struct NodoArbol * derecha){
    
    struct NodoArbol * nuevoNodo = crearNuevoNodo(izquierda, elemento,letra, derecha);
    raiz = nuevoNodo;
}
...
void insertar(struct ArbolBinarioBusqueda & arbol, int elemento,char letra){
    insertarRecursivo(arbol.arbolBinario.raiz, elemento,letra);
}

void insertarRecursivo(struct NodoArbol *& raiz, int elemento,char letra){
    if(esNodoVacio(raiz))
        plantarArbolBinarioB(raiz, nullptr, elemento,letra, nullptr);
    else
        if(raiz->elemento > elemento)
            insertarRecursivo(raiz->izquierda, elemento,letra);
        else
            if(raiz->elemento < elemento)
                insertarRecursivo(raiz->derecha, elemento,letra);
            else
