...
/*Parte b*/
/*Crea un nuevo nodo con los datos dados como parámetros*/
struct Nodo * crearNodoNew(struct NodoArbol *nodoArbol, struct Nodo * siguiente) {
    struct Nodo * nuevoNodo = new struct Nodo;
...
void insertarencolaNew(struct Lista & tad, NodoArbol *nodoArbol) {
    struct Nodo * ultimoNodo = tad.cola;
    struct Nodo * nuevoNodo = crearNodoNew(nodoArbol, nullptr);
...
NodoArbol *retornaCabezaNew(const struct Lista & tad){
    return tad.cabeza->nodoArbol;
}