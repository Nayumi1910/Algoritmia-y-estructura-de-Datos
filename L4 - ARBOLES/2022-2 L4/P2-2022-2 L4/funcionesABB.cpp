...
void insertar(struct ArbolBinarioBusqueda & arbol, int elemento, int cant){
    insertarRecursivo(arbol.arbolBinario.raiz, elemento,cant);
}

void insertarRecursivo(struct NodoArbol *& raiz, int elemento,int cant){
    if(esNodoVacio(raiz))
        plantarArbolBinario(raiz, nullptr, elemento,cant, nullptr);
    else
        if(raiz->elemento > elemento)
            insertarRecursivo(raiz->izquierda, elemento,cant);
        else
            if(raiz->elemento < elemento)
                insertarRecursivo(raiz->derecha, elemento,cant);
            else
