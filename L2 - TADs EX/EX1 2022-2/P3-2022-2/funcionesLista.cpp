...
Nodo* ubicarAnterior( Lista L, int e)
{
    Nodo *p, *pA;
    pA = NULL;
    p = L.cabeza;
...
Nodo* ubicarElemento( Lista L, int e)
{
    Nodo *p;
    if ( esListaVacia(L) )
        return NULL;
...
void eliminar( Lista &pL, int e )
{
    ...
    else
    {
        ...
        if ( pA == NULL )
            pL.cabeza = p->siguiente;
        else
            pA->siguiente = p->siguiente;
        pL.longitud--;
        delete p;
    }
}
...
void imprime(const struct Lista & lista){
    ...
    if (esListaVacia(lista)){
        cout<<"No tiene numero tachados"<<endl;
    }
    else{
