...
struct Nodo * crearNodo(int hora, int sucursal, struct Nodo * siguiente){
...
void insertarAlInicio(struct Lista & tad, int hora, int sucursal) {
...
void insertarAlFinal(struct Lista & lista, int hora, int sucursal){
    struct Nodo * nuevoNodo = crearNodo(hora, sucursal, nullptr);
...
void imprime(const struct Lista & lista){
    ...
    if (esListaVacia(lista)){
        ...
        while(recorrido != nullptr){
            cout<<recorrido->hora<<"("<<recorrido->sucursal<<")"<<" ";
            recorrido = recorrido->siguiente;
        }   
    }
    cout<<endl;
