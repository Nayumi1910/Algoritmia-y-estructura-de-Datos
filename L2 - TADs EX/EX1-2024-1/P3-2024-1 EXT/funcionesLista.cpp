...
void imprime(const struct Lista & lista){
    ...
    if (esListaVacia(lista)){
        ...
        cout<<setw(10)<<"Avio"<<setw(15)<<"Hora llegada"
                <<setw(15)<<"Nueva hora"<<endl;
        struct Nodo * recorrido = lista.cabeza;
        while(recorrido != nullptr){
            cout<<setw(10)<<recorrido->elemento.numVuelo<<" "
                    <<setw(10)<<recorrido->elemento.inicio.hora<<":"
                    <<recorrido->elemento.inicio.min<<" "
                    <<setw(10)<<recorrido->elemento.fin.hora<<":"<<
                    recorrido->elemento.fin.min<<endl;
            recorrido = recorrido->siguiente;
        }   
    }
    cout<<endl;
