...
struct Nodo * crearNodo(int hora, int dia, struct Nodo * siguiente){
...
void insertarAlFinal(struct Lista & lista, int hora, int dia){
    struct Nodo * nuevoNodo = crearNodo(hora, dia, nullptr);
...
void imprime(const struct Lista & lista){
    ...
    if (esListaVacia(lista)){
        ...
        while(recorrido != nullptr){
            cout<<recorrido->hora<<"("<<recorrido->dia<<")";
